        ...
        <Card className={`p-8 ${classes.cardHeight}`} variant="outlined">
            ...
            <Box className='pt-16'>
                <Typography component="div">
                    ...
                    <Box fontSize={14} m={1} className='text-muted'>{data.email}</Box>
                </Typography>
            </Box>
            <Box className='pt-24'>
                <Typography component="div">
                    <Box fontSize={14} fontWeight="fontWeightMedium" m={1}>{t("lbl_workphone")}</Box>
                    <Box fontSize={14} m={1} className='text-muted'>{data.contact}</Box>
                </Typography>
            </Box>
            <DropDownComponent lable={`${t("lbl_timelog")}`} setValue={useCallback((e) => setTimelog(e), [])} data={data.timeLog} value={timelog} />
            <DropDownComponent lable={`${t("lbl_todo")}`} setValue={useCallback((e) => setTodo(e), [])} data={data.Todo} value={todo} />
        </Card>
    )
}
